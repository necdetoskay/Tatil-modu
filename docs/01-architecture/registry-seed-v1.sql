...
-- Idempotent by canonical code. No destructive deletes.
...
ON CONFLICT (code) DO UPDATE SET rank=EXCLUDED.rank,is_active=EXCLUDED.is_active;
...
ON CONFLICT (code) DO UPDATE SET allows_targetless=EXCLUDED.allows_targetless,is_active=EXCLUDED.is_active;
...
ON CONFLICT (code) DO UPDATE SET value_type=EXCLUDED.value_type,is_active=EXCLUDED.is_active;

INSERT INTO freshness_policies (code,ttl_seconds,refresh_priority,requires_live_verification,is_active) VALUES
('very_static',15552000,60,false,true),
('static',7776000,50,false,true),
('slow_change',2592000,40,false,true),
('operational',604800,30,false,true),
('event_schedule',259200,20,true,true),
('near_live',21600,10,true,true),
('live',1800,0,true,true)
ON CONFLICT (code) DO UPDATE SET ttl_seconds=EXCLUDED.ttl_seconds,refresh_priority=EXCLUDED.refresh_priority,requires_live_verification=EXCLUDED.requires_live_verification,is_active=EXCLUDED.is_active;

-- Root place categories
...
-- Child categories: parent resolved by canonical code.
...
ON CONFLICT (code) DO UPDATE SET parent_category_id=EXCLUDED.parent_category_id,is_active=EXCLUDED.is_active;
